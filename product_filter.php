...
function validateData($data) {
    ...
    if ((!empty($data['price_min']) && !preg_match("/^[0-9]+$/", $data['price_min'])) || (!empty($data['price_max']) && !preg_match("/^[0-9]+$/", $data['price_max']))) {
        $errors['price'] = 'Неверное значение поля "Цена"';
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    setcookie('product_data', json_encode($form_data), 0, '/');
...
function executeSql() {
    try {
        ...
        if (!empty($_POST['price_min'])) {
            $sql .= ' AND Цена_закупки >= :param1';
            $params[':param1'] = $_POST['price_min'];
        }

        if (!empty($_POST['price_max'])) {
            $sql .= ' AND Цена_закупки <= :param2';
            $params[':param2'] = $_POST['price_max'];
        }
